    ...
    void solve()
    {
        ...
        vi minus_available;
        int num_of_ones=0;
        int total_minus_available=0;
        //  initialize both arrays with zero at starting index
        ones_array.push_back(0);
        minus_available.push_back(0);
        ...
        FORI(i,n)
            {
                ...
                ones_array.push_back(num_of_ones);
            }
            FORI(i,n)
            {
                if(a[i]>=2)
                {
                total_minus_available+=a[i]-1;
                }
                minus_available.push_back(total_minus_available);
            }
            // print both arrays
            // pt_(ones_array);
            // pt_(minus_available);
...

        {
            ...
            int extras=0;
            // find out number of ones in  l to r
            
        
    
            
            if(minus_available[r]-minus_available[l-1]>=ones_array[r]-ones_array[l-1])
            {
                // print total_minus_available;
                // cout << total_minus_available << endl;
                cout<<"YES"<<endl;
            }
            else
            {
