...
void solve()
{
    ...
    FORI(i, n)
    {
        ...
        if (s.back() < a[i] && r.size()==0)
        {
            r.push_back(a[i]);
            continue;
        }
        else if (s.back() >=a[i] && r.size()==0)
        {
            s.push_back(a[i]);
            continue;
        }
        if (a[i] <= s.back() && a[i] <= r.back())
        {
            if (s.back() > r.back())
            {
                r.push_back(a[i]);
    ...
    // count ans according to the number of pair in s  where s[i]<s[i+1] in loop
    ...
    // count ans according to the number of pair in r  where r[i]<r[i+1] in loop
